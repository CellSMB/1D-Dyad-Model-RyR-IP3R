...
simWidth=8; % um
c0=0.1;
casr0=1000*ones(numSimulations,1);
IP3Conc=0.15;
vary_rslt_name={'Young','Old'};
vary_SERCA_lvl=linspace(1,2^-0.53233,numel(vary_rslt_name));

for run=1:numel(vary_SERCA_lvl)
    SERCA_lvl=vary_SERCA_lvl(run);
    rslt_name=vary_rslt_name{run};
    nIP3R=0;
    parfor(fop=1:numel(casr0),12)
        ...
        par.sc=1000;
        par.scale_sercas=SERCA_lvl; % to reflect change in SERCA expression
        par.gryr=2.8;
        par.grefill=0.2;
        par.dt0=1e-4; %ms
        par.dx=0.2/5; %um
        ...
        h24_inf=ip3r_par.kn24^ip3r_par.nn24./(ip3r_par.kn24^ip3r_par.nn24+(c0.*isoform_scale).^ip3r_par.nn24);
        h24=h24_inf*ones(total_IP3Rs,1);
        
        m24_inf=(c0.*isoform_scale).^ip3r_par.n24./(ip3r_par.k24^ip3r_par.n24+(c0.*isoform_scale).^ip3r_par.n24);
        m24=m24_inf*ones(total_IP3Rs,1);
        
        m42_inf=(c0.*isoform_scale).^ip3r_par.n42./(ip3r_par.k42.^ip3r_par.n42+(c0.*isoform_scale).^ip3r_par.n42);
        m42=m42_inf*ones(total_IP3Rs,1);
        
        h42_inf=ip3r_par.kn42^ip3r_par.nn42./(ip3r_par.kn42^ip3r_par.nn42+(c0.*isoform_scale).^ip3r_par.nn42);
        ...
        mid_cell=ceil(linescan_length/4*2); % for uncoupled one
        ...
        RyR_sparse(IP3RVector,3)=4; % randi([1,6],numel(IP3RVector),1);
        RyR_sparse(:,2)=1;          % simplified receptor state
        Ch_mx=[-1;1];   % used to change channel state
        ...
        for iter=1:titer_total
            ...
            while dumt<=(par.dt0*100)
                ...
                tanVector = [[zeros(total_RyRs,1) ...
                    min(4.57*1e2*((Yn(RyR_sparse(RyRVector,1),1)./par.sc).^2.12),0.800) ...
                    0.245*((Yn(RyR_sparse(RyRVector,1),1)./par.sc).^-0.27) ...
                    zeros(total_RyRs,1)];...
                    [zeros(total_IP3Rs,1)...
                    sum(IP3Rs_q_Old,2)... 
                    sum(IP3Rs_q_Old,2)... 
                ...
                tanNewVector = [[zeros(total_RyRs,1)...
                    min(4.57*1e2*((Yn1(RyR_sparse(RyRVector,1),1)/par.sc).^2.12),0.800) ...
                    0.245*((Yn1(RyR_sparse(RyRVector,1),1)/par.sc).^-0.27) ...
                    zeros(total_RyRs,1)];...
                    [zeros(total_IP3Rs,1)...
                    sum(IP3Rs_q_New,2)... 
                    sum(IP3Rs_q_New,2)... 
        ...
        parsave(['one_d_Cannell_Cao_Sheep_',rslt_name,'_ryrs_',num2str(total_RyRs),'_ip3rs_',num2str(total_IP3Rs),'_Tsim_',num2str(simTime),'smaller_set_test_v',num2str(fop),'_tr13_try_initiate_LTCC.mat'],time,Cai_save,CaJSR_save,0.,CaF4_save,RyR_open_save,IP3R_open_save,par,ip3r_par)
