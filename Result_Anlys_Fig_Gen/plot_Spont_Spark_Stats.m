...
% Stores n(runs) of unique n(sparks)
Uniq_Spark_Count=cell(numel(st_t),numel(IP3R_Names));
min_nSpark=1;

% Stores n_O(RyR) that does not lead to a spark
nRyR_Open_NoSpark=cell(numel(st_t),numel(IP3R_Names));
...
for TriggerType=1:numel(st_t) % 3 stimulation types
    for IP3R_Indx=1:numel(IP3R_Names)
        ...
        Uniq_Spark_Count{TriggerType,IP3R_Indx}=uniq_nSpark_count(min_nSpark+1:end)';  % min_nSpark+1 to exclude n(spark)=0 data
        ...
        nRyR_Open_NoSpark{TriggerType,IP3R_Indx}=file_struct.RyR_OpenNoSpark;
    end
end
clear file_struct;
...
% bar graphs for n(sims) with sparks & mean n(sparks)
max_nSpark=max(max(cell2mat(cellfun(@numel,Uniq_Spark_Count,'UniformOutput',false))));  % find max n_Spark out of all
Uniq_Spark_Count=cellfun(@(M) [M;zeros(max_nSpark-numel(M),1)],Uniq_Spark_Count,'UniformOutput',false);  % ensures all Spark_Count have same length
nSpark_bar_legendCell=cellstr(num2str((min_nSpark:max_nSpark)','%g Sparks'));    % (2:end-1) to exclude n(spark)=0 and last data
figure(FigNum);
bar_xlabel1=categorical(bar_xNames1);
bar_xlabel1=reordercats(bar_xlabel1,bar_xNames1);
bar_data_prep=double(cell2mat(Uniq_Spark_Count))'/nSim_Total*100;
handle1=bar(bar_xlabel1,bar_data_prep,'stacked','EdgeColor','none');
set(handle1,{'DisplayName'},nSpark_bar_legendCell)
legend()
legend boxoff
ylim([0 ceil(max(sum(bar_data_prep,2))/10)*10])
xtickangle(30)
set(gca,'FontSize',TickFontSize,'FontName',FontName)
ylabel(['Simulations with ',char(8805),'1 Ca^{2+} spark (%)'],'FontSize',LabelFontSize)
grid off
box off
set(gcf,'color','w','units','centimeters','position',FigureSize);
saveas(gcf,['one_dyad_spark_percentage_barchart_', Output_file_remark, 'plots.fig'])
FigNum=FigNum+1;
...
% bar graph of n(no-spark RyR openings)
figure(FigNum)
bar_xlabel1=categorical(bar_xNames1);
bar_xlabel1=reordercats(bar_xlabel1,bar_xNames1);
nNoSpark_raw_data=cell2mat(nRyR_Open_NoSpark);
nNoSpark_mean_data=mean(nNoSpark_raw_data)';
nNoSpark_stErr_data=std(nNoSpark_raw_data)'/sqrt(length(nNoSpark_raw_data));
bar(bar_xlabel1,nNoSpark_mean_data,'EdgeColor','none');
hold on
errorbar(bar_xlabel1,nNoSpark_mean_data,nNoSpark_stErr_data,'Color','k','LineStyle','none','LineWidth',1);
hold off
xtickangle(30)
set(gca,'FontSize',TickFontSize,'FontName',FontName)
ax.FontSize = TickFontSize;
ax.FontName = FontName;
ylabel('Average n(no-spark RyR openings)','FontSize',LabelFontSize)
grid off
box off
set(gcf,'color','w','units','centimeters','position',FigureSize);
saveas(gcf,['one_dyad_NoSpark_RyR_Opening_', Output_file_remark, 'plots.fig'])
FigNum=FigNum+1;
...
ylabel('n(IP_3R)')
saveas(gcf,['one_dyad_FDHM_ANOVA_', Output_file_remark, 'plots.fig'])
...
ylabel('n(IP_3R)')
saveas(gcf,['one_dyad_Amp_ANOVA_', Output_file_remark, 'plots.fig'])
