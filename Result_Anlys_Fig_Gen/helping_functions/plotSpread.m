...
if ~isempty(varargin) && ~ischar(varargin{1}) && ~isstruct(varargin{1})
    ...
    parserObj.addOptional('categoryLabels',def.categoryLabels);
    ...
    switch opt.xyOri
        case 'normal'
            if any(opt.showMM==[1,2])
                mh = plot(ah,opt.xValues,m,'_','Color','k','MarkerSize',12,'LineWidth',1);
            end
            if any(opt.showMM==[1,3])
                mdh = plot(ah,opt.xValues,md,'sg','MarkerSize',12,'LineWidth',1);
            end
            if opt.showMM == 4
                mh = plot(ah,opt.xValues,m,'_','Color','k','MarkerSize',12,'LineWidth',1);
        ...
        case 'flipped'
            if any(opt.showMM==[1,2])
                mh = plot(ah,m,opt.xValues,'_','Color','r','MarkerSize',12,'LineWidth',1);
            end
            if any(opt.showMM==[1,3])
                mdh = plot(ah,md,opt.xValues,'sg','MarkerSize',12,'LineWidth',1);
            end
            if opt.showMM == 4
                mh = plot(ah,m,opt.xValues,'_','Color','r','MarkerSize',12,'LineWidth',1);
